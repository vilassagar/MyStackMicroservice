...
namespace AuthService.Repository
{
    public class PermissionRepository : GenericRepository<Permission>, IPermissionRepository
    {
        ...
        {
            return await _dbSet
                .Where(p => p.UserPermissions.Any(up => up.UserId == userId) && !p.IsDeleted)
                .ToListAsync();
        }
        ...
        {
            return await _dbSet
                .Where(p => p.Resource == resource && !p.IsDeleted && !p.IsDeleted)
                .ToListAsync();
        }
        ...
        {
            return await _dbSet
                .Where(p => p.Action == action && !p.IsDeleted && !p.IsDeleted)
                .ToListAsync();
        }
        ...
        {
            return await _dbSet
                .FirstOrDefaultAsync(p => p.PermissionName == permissionName && !p.IsDeleted && !p.IsDeleted);
        }
        ...
        public async Task<bool> UserHasPermissionAsync(int userId, string permissionName)
        {
            ...
            var hasRolePermission = await _context.Set<ApplicationUser>()
                ...
                .AnyAsync(p => p.PermissionName == permissionName && !p.IsDeleted && !p.IsDeleted);
        ...
        public async Task<IEnumerable<Permission>> GetByIdsAsync(List<int> permissionIds, bool includeInactive = false)
        {
            try
            {
                ...
                if (!includeInactive)
                {
                    query = query.Where(p => !p.IsDeleted); // Assuming you have an IsActive property
                }
